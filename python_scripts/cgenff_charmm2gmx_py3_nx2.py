...
def check_versions(str_filename, ffdoc_filename):
    ...
    for line in f.readlines():
        if line.startswith("Parameters taken from CHARMM36 and CGenFF"):
            entry = re.split('\s+', line.lstrip())
            ffver = entry[6]
            print("--Version of CGenFF detected in ",
                  ffdoc_filename, ":", ffver)
    f.close()
    ...
    for line in nocomments:
        if state == "free":
            ...
            # jal - adding lone pair support
                ...
                topology["RESI"][resname]["vsites"].append(
                    (name, at1, at2, dist))
    ...
    for line in nocomments:
        ...
        if section == "BOND":
            ...
            if line.find('!'):
                line = line[:line.find('!')]
            s = line.split()
            ai, aj, ak, al, k, n, d = s[0], s[1], s[2], s[3], float(
                s[4]), int(s[5]), float(s[6])
            parameters["DIHE"].append([ai, aj, ak, al, k, n, d])
        elif section == "IMPR":
            if line.find('!'):
                line = line[:line.find('!')]
            s = line.split()
            ai, aj, ak, al, k, d = s[0], s[1], s[2], s[3], float(
                s[4]), float(s[6])
            ...
            if len(prm) > 4:
                ...
                parameters["NONBONDED14"].append(
                    (atname, epsilon14, half_rmin14))
            ...
            if line.find('!'):
                ...
                if line.startswith("BOND") or line.startswith("DOUB"):
                    ...
                    for bondi in range(0, numbonds):
                        ...
                        if(not found1):
                            print(
                                "Error:atomgroup:read_charmm_rtp> Atomname not found in top", entry[(bondi*2)+1])
                        if(not found2):
                            print(
                                "Error:atomgroup:read_charmm_rtp> Atomname not found in top", entry[(bondi*2)+2])
                        # jal - ignore "bonds" to lone pairs
                        if ((is_lp(atm[i]['name']) == False) and (is_lp(atm[j]['name']) == False)):
                            self.G.add_edge(i, j)
                            # treat all bonds as single for now
                            self.G[i][j]['order'] = '1'
